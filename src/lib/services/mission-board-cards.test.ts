...
describe('summarizeCompletionEvidenceForBoard', () => {
	it('summarizes missing completion evidence without exposing raw field names', () => {
		expect(summarizeCompletionEvidenceForBoard({
			state: 'incomplete',
			...
			hasArtifactReference: false,
			tasksTerminal: true
		})).toBe('Missing 3 completion signals');
	});

	it('keeps complete and non-terminal evidence quiet on the board', () => {
		expect(summarizeCompletionEvidenceForBoard({
			state: 'complete',
			summary: 'Completion evidence present.',
			missing: [],
