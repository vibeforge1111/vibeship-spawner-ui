...
/**
 * Load a workflow template directly onto the canvas
 * @param template The workflow template to load
 * @param skills Array of available skills to match against
 * @param clearFirst Whether to clear the canvas before loading (default: true)
 */
export function loadWorkflowTemplate(
	template: {
		nodes: { skillId: string; offsetX: number; offsetY: number }[];
	...
	for (const templateNode of template.nodes) {
		const skill = skills.find((s) => s.id === templateNode.skillId);
		if (!skill) {
	...
	return addNodesWithConnections(nodeDefs, template.connections);
}
