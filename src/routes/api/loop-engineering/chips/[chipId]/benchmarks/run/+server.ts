...
export const POST: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		const result = executeNow
			? await executePrivateBenchmarkRun({
				chipKey,
		...
		return json({
			...
			...('benchmarkRun' in result ? { benchmarkRun: result.benchmarkRun } : {}),
		...
		return json({ ok: false, error: errorMessage(error, 'benchmark run launch failed') }, { status: 400 });
	}
};
