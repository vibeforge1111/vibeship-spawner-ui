...
fixed: Completed PRD-to-pipeline polling fallback implementation. The events endpoint now stores prd_analysis_complete results to file, and Welcome.svelte polls for results every 2 seconds. This ensures pipeline creation works even when SSE events are missed.
learned: BUG FIX: PRD pipeline creation failed because Welcome.svelte used goto('/canvas') for client-side navigation, but this caused Svelte reactivity issues. The regular mode uses window.location.href which does a full page reload. Fixed by changing PRD mode to also use window.location.href. The comment in +page.svelte explicitly warned about this: "Client-side navigation causes canvas to become unresponsive".
decided: PRD pipeline layout redesigned: Task-focused nodes now show feature names (e.g., "Hand Tracking Integration") instead of duplicate skill names (e.g., "game-design" 20 times). Skills are now shown as teal-colored tags underneath each task node. Changes: (1) prdResultToWorkflow in Welcome.svelte creates task nodes with name=task.title and skillChain=task.skills, (2) SkillNode.svelte displays up to 4 skill tags with "+X" overflow. This gives users a cleaner view of WHAT is being built and WHICH skills handle each task.
fixed: Implemented P0 Three.js improvements for EnvForge: 1) Added HDRI Environment support with auto-preset detection based on scene content (city, forest, sunset, etc.). 2) Added post-processing effects (SSAO, Bloom, Vignette, ToneMapping) with UI toggles. These improvements dramatically enhance PBR material appearance and add professional visual quality.
fixed: PRD analysis completed for "The Chaos Garden" - anti-fragile dev system. Generated 23 tasks across 7 phases with 32 matched H70 skills. Sent to Spawner UI successfully (evt-1768431602323-do5pyk).
learned: PRD Analysis Bridge - CRITICAL: Always fetch the CURRENT pending requestId from `/api/prd-bridge/pending` before sending results. The UI generates a new requestId each time the user submits a PRD, so stale requestIds will be ignored. Flow: 1) Check pending endpoint for current requestId, 2) Use THAT requestId in the response, 3) Send to /api/events. The UI polls for results matching its specific requestId every 2 seconds.
decided: Added CRITICAL requestId matching rules to CLAUDE.md for PRD analysis bridge. The rule documents the exact flow: 1) Fetch current requestId from /api/prd-bridge/pending, 2) Use that exact requestId in response, 3) Send to /api/events. This prevents the common failure where Claude analyzes with an old requestId that gets ignored by the UI.
fixed: Completed Chaos Garden project - all 23 tasks done. Fixed TypeScript errors in endpoint-discovery.ts, fuzzer.ts, and pr-creation.ts. Added getRepoTree and getHeaders methods to GitHubClient. Configured dual adapters (node for Docker, vercel for Vercel). Build successful.
fixed: SupaShield Ralph Wiggum benchmark completed: 105 attack vectors, 100% detection rate across 5 vulnerable Supabase test scenarios (No Security, Bad RLS, Business Logic, Vibecoder, Advanced). Created 24 tests covering RLS bypass, IDOR, injection, SSRF, default creds, and data exposure.
fixed: SupaShield Ralph Wiggum Full Benchmark completed: 100% detection rate across 49 tests in 10 scenarios. All 252 attack vectors validated against deliberately vulnerable Supabase configurations covering RLS bypass, Auth, AI/ML, Multi-tenant, Backup, Logging, and more.
fixed: SupaShield benchmark complete: Created Docker Supabase test environment with 10 intentional vulnerability scenarios (NO RLS, USING(true), missing operations, horizontal privilege escalation, price manipulation, IDOR, SQL injection, storage misconfig, auth bypass, realtime exposure). Scanner detected 14 vulnerabilities across 9 tables. Created comprehensive fix rulesets (src/lib/engine/fixes/index.ts) with SQL scripts for: rls-no-rls, rls-using-true, rls-missing-operations, auth-weak-password, auth-rate-limit, api-security-headers, api-cors, api-graphql-introspection, logic-quantity-abuse, data-sensitive-exposure. Verified fixes work by applying RLS to users_no_rls table - now returns empty array instead of sensitive data. Added SQL fix viewing and download functionality to dashboard.
fixed: PRD pipeline loading bug - canvas now checks for pending PRD results on mount. Problem: If user navigated to canvas before Claude's PRD analysis completed, Welcome component unmounted and polling stopped. Result was stored but never processed, so canvas loaded OLD pipeline. Fix: (1) Added checkPendingPRDResult() to prd-bridge.ts, (2) Canvas onMount now calls it FIRST before loading any pipeline, (3) If pending result found, creates new pipeline and loads nodes, (4) Extracted prdResultToWorkflow() to shared service. Key files: src/lib/services/prd-bridge.ts (checkPendingPRDResult, prdResultToWorkflow), src/routes/canvas/+page.svelte (onMount pending check).